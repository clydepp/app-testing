...
async def process(websocket):
    async for message in websocket:
        try:
            # ✅ FIXED: Decode JPEG data instead of raw bytes
            # Convert message bytes to numpy array
            jpeg_bytes = np.frombuffer(message, dtype=np.uint8)
            
            # Decode JPEG using OpenCV
            recv = cv2.imdecode(jpeg_bytes, cv2.IMREAD_COLOR)
            ...
            recv = cv2.cvtColor(recv, cv2.COLOR_BGR2RGB)
            
            # ✅ Send processed frame to UI
            await send_to_ui(recv)
            print("processed and sent decoded frame")
...
async def send_to_ui(frame):
    ...
    if ui_client:
        try:
            ...
            im_file = BytesIO()
            image.save(im_file, format='JPEG', quality=70)
            jpeg_bytes = im_file.getvalue()
            await ui_client.send(jpeg_bytes)
            print("frame sent to UI in JPEG format")
        except Exception as e:
